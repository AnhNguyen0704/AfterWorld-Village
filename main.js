...
export const LOOK = {
  ...
  camera: { fovDeg: 40, heightM: 1.7, minPolarDeg: 35, maxPolarDeg: 84, minDistM: 4, maxDistM: 48 },
...
const VIEWS = {
  'Overview': { position: [26, 18, 28], target: [0, 1.6, 0] },
  'Village Path': { position: [-2, 3.3, 12], target: [-2, 1.6, -1] },
  'Main House': { position: [2.1, 3, 4], target: [-2.4, 1.7, -3.1] },
  'Pond & Bridge': { position: [1.2, 3.4, 11], target: [4, .8, 6.1] },
  'Lake & Forest': { position: [23, 7, 18], target: [16, .6, 11] },
  'Close-up Detail': { position: [-5.5, 2.1, .2], target: [-2.4, 1.55, -3.1] }
};
...
document.querySelector('#lighting').addEventListener('click', (event) => { const on = event.currentTarget.getAttribute('aria-pressed') !== 'true'; world.setLighting(on); event.currentTarget.setAttribute('aria-pressed', String(on)); event.currentTarget.textContent = on ? 'Golden hour on' : 'Golden hour low'; invalidate(); });
new ResizeObserver(() => {
